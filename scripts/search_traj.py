...
# Import the collector and reconstructor as libraries. load.py sets the SDL
# software-render env vars and imports habitat at module load; harmless here
# (no pygame window is ever opened). reconstruct.py pulls in open3d.
...
def mean_l2(pred_poses, gt_poses):
    """Mean L2 distance between predicted and GT camera centers — the metric
    reconstruct.visualize_and_evaluate reports, without the o3d window."""
    if gt_poses is None or len(gt_poses) == 0 or len(pred_poses) == 0:
        return float("inf")
    n = min(len(pred_poses), len(gt_poses))
    pred_c = np.array([p[:3, 3] for p in pred_poses[:n]])
    gt_c = np.array([g[:3, 3] for g in gt_poses[:n]])
    return float(np.mean(np.linalg.norm(pred_c - gt_c, axis=1)))


def score_capture(data_root, floor, version):
    """Reconstruct the captured run and return its mean L2 error (lower=better)."""
    args = SimpleNamespace(data_root=data_root, floor=floor, version=version)
    _, pred_poses, gt_poses = rec.reconstruct(args)
    return mean_l2(pred_poses, gt_poses)
