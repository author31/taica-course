...
ONTOLOGY CONTRACT  (must match ontology/hw1.ttl — see NS/HW1 below)
    Classes: Batch, Frame, Component, RGBImage, DepthImage, QualityFactor.
    Object props: hasFrame (Batch->Frame), hasComponent (Frame->Component).
...
def build_batch_graph(data_dir, floor, include_ontology=True):
    ...
    for stem, rgb_path, depth_path in pairs:
        ...
        g.add((f, HW1.avgLuma, Literal(frame_luma(rgb_path), datatype=XSD.double)))
        g.add((f, HW1.validDepthFraction,
        ...
        g.add((rc, HW1.imagePath, Literal(rgb_path)))
        g.add((f, HW1.hasComponent, rc))

        dc = _component_iri(name, idx, "depth")
        g.add((dc, RDF.type, HW1.DepthImage))
        g.add((dc, HW1.imagePath, Literal(depth_path)))
        g.add((f, HW1.hasComponent, dc))
